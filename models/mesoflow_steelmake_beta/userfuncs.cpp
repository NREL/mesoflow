...
namespace mflo_user_funcs
{
    ...
    AMREX_GPU_DEVICE_MANAGED Real temp_left       = 800.0;
...
//   AMREX_GPU_DEVICE_MANAGED Real spec_left[NUM_GAS_SPECIES] = {0.5, 0.3, 0.0, 0.0, 0.2}; // Example values for H2, CO, H2O, CO2, N2
    ...
    void initialize_problem()
    {
        ...
        pp.query("H2",H2molfrac);
        pp.query("H2O",H2Omolfrac);
        pp.query("CO",COmolfrac);
        pp.query("N2",N2molfrac);
        
        pp.query("mrcfile",mrcfile);
        pp.query("mrc_threshold",mrc_threshold);

//        //concentration
//        spec_left[H2_ID] = 0.5 * dens_left / mflo_species::molwts[H2_ID];
//        spec_left[CO_ID] = 0.25 * dens_left / mflo_species::molwts[CO_ID];
//     //   spec_left[H2O_ID] = 0.25 * dens_left / mflo_species::molwts[H2O_ID];
